...
/**
 * Command to search for outgoing transactions
 */
public class SearchOutgoingTransactionCommand extends Command {
    public SearchOutgoingTransactionCommand(TransactionManager transactionManager) {
        super("Search outgoing transaction", transactionManager);
    }
    ...
    public void execute(Scanner scanner) {
        ...
        while (total == -1) {
            ...
            if (selection == 1) {
                total = searchByYear(scanner, TransactionType.OUTGOING);
                System.out.println("Total is " + total + "kr.");
            } else if (selection == 2) {
                total = searchByMonth(scanner, TransactionType.OUTGOING);
                System.out.println("Total is " + total + "kr.");
            } else if (selection == 3) {
                total = searchByWeek(scanner, TransactionType.OUTGOING);
                System.out.println("Total is " + total + "kr.");
            } else if (selection == 4) {
                total = searchByDay(scanner, TransactionType.OUTGOING);
                System.out.println("Total is " + total + "kr.");
            } else {
                System.out.println("Enter a valid number");
                selection = -1;
